function [denoised,window,GUE_MSE] = den_win(Mmax,p,noisy_sig,type,sigma)

%% Denoising Using Optimum length SG filter
...
%%%% Output
    ...
    % GUE_MSE: GUE-MSEcorresponding to Mmin to Mmax, at each time instants
...
Mmin=2; %% minimum half-windowlength 
...
j=1;
 for t=Mmin:Mmax
    GUE_MSE(j,:)=Find_risk_win(H{j},A{j},2*t+1,sigma,noisy_samp,Mmax,h);
    j=j+1;
 end
